...
django-docutils renders [reStructuredText] to HTML and returns that HTML as
safe template output. The default renderer is locked down for content that may
come from users, editors, uploads, API requests, or other lower-trust sources.
...
HTML generation; the {rst:role}`kbd` role renders escaped inline text instead
of raw HTML.
